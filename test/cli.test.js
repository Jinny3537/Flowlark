...
describe('CLI 全流程', () => {
  ...
  test('milestone 预检与本地状态流转可通过 CLI 验证', (t) => {
    const dir = workspace()
    ph(dir, 'init')
    ...
    t.assert.strictEqual(r.code, 1)
    t.assert.match(r.err, /--confirm/)
    r = ph(dir, 'milestone', 'transition', 'S1', 'canceled', '--reason', '范围取消', '--confirm')
    t.assert.strictEqual(r.code, 0, r.err)
    t.assert.strictEqual(JSON.parse(ph(dir, 'milestone', 'show', 'S1', '--json').out).status, 'canceled')
  })
})
