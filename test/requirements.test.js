...
test('扩展需求字段支持创建、局部更新、清空且不影响本地截止日期', (t) => {
  const { root, hub } = newHub()
  t.after(() => cleanup(root))
  hub.createRequirement({ code: 'REQ-FIELDS', title: '字段验证', businessValue: '业务价值', businessRule: '业务规则', acceptanceCriteria: '- [ ] 验收', versionName: '九月发布', targetDeliveryDate: 'TBD', expectedOnlineDate: '待确认', dueDate: '2026-09-30' })
  hub.updateRequirement('REQ-FIELDS', { title: '更新标题' })
  let item = hub.getRequirement('REQ-FIELDS')
  t.assert.equal(item.businessRule, '业务规则')
  t.assert.equal(item.targetDeliveryDate, 'TBD')
  t.assert.equal(item.expectedOnlineDate, '待确认')
  t.assert.equal(item.dueDate, '2026-09-30')
  hub.updateRequirement('REQ-FIELDS', { businessRule: '', versionName: '', businessValue: '新背景' })
  item = hub.getRequirement('REQ-FIELDS')
  t.assert.equal(item.businessRule, '')
  t.assert.equal(item.versionName, '')
  t.assert.equal(item.businessValue, '新背景')
})
