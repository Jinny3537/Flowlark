...
- 归档关联支持每条需求的页面位置和承载范围；需求与归档原型双向跳转，直接预览具体版本。
- 需求详情以协作和开发依据为入口，区分迭代明确采用版本与最新归档，支持安排候选迭代范围。
- 需求聚合跨版本问题、研发进展与验收记录；处理回复关联外部修改后的归档版本，研发/测试需在该版本确认。
- 本机标注草稿可转为可追踪协作问题，保存文字与坐标；沿用已有角色与服务端权限校验。
- 增加本地分析和变更历史、同步差异预览、过期预览校验、失败项重新预览重试。
- 支持单条归档、软删除、回收站恢复及 CLI 对应操作。已删除编号同步排除；冻结/进行中迭代引用阻止删除；保留历史与原型文件。
- 需求列表增加本地/其他外部状态分类、待处理筛选、URL 筛选保留及原型入口。
- Git 受管范围补齐 requirements/ 和 milestones/，确保协作依据与需求数据一起同步。
