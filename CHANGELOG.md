...
- Schema 3 迁移在改写元数据前创建本机备份，失败时可恢复；项目新增默认为 `manual` 的 `project.sync` 准备策略。
- 迭代同步预览会持久化为本机可恢复队列记录，同步中心可审阅差异、确认执行、查看步骤与脱敏审计，并在失败后人工重试。
- 同步审计以 append-only 方式写入团队跟踪文件，密码、授权信息、Token、Secret 和环境变量值递归脱敏。
- 同步中心 API 只提供服务端固定的执行、重试和取消动作，不接受浏览器指定 MCP 工具或任意远端操作；`trusted-auto` 在本版仅保存准备数据。
