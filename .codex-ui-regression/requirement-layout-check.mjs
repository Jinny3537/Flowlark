...
hub.createRequirement({ code: 'REQ-FIELDS', title: '需求字段验证', external: { provider: 'mcp', status: '待评审' }, project: '海丰和泰', owner: '产品负责人', priority: 'P2', stage: '需求分析', analysisStatus: '待分析', versionName: '九月发布', businessValue: '减少重复录入', description: '支持完整同步', rawDescription: '业务原文', businessRule: '保留原始业务规则', acceptanceCriteria: '- [ ] 完整保留字段\n- [x] 可以查看详情', expectedOnlineDate: 'TBD', targetDeliveryDate: '待确认', functionPoints: '需求同步', impactScope: '需求模块' })
hub.createRequirement({ code: 'REQ-OTHER', title: '其他阶段', external: { provider: 'mcp', status: '开发中' }, stage: '开发中', priority: 'P0' })
...
try {
  ...
  await page.getByRole('tab', { name: '待评审（1）', exact: true }).click()
  assert.equal(await page.getByRole('button', { name: 'REQ-FIELDS', exact: true }).count(), 1)
  await page.getByRole('tab', { name: '已上线（0）', exact: true }).click()
  await page.getByText('没有匹配的需求', { exact: true }).waitFor()
  await page.getByRole('tab', { name: '全部（2）', exact: true }).click()
  await page.getByRole('searchbox', { name: '搜索需求', exact: true }).fill('REQ-OTHER')
  await page.getByRole('tab', { name: '开发中（1）', exact: true }).click()
  ...
  await page.getByRole('tab', { name: '全部（2）', exact: true }).waitFor()
  ...
  await page.getByRole('tab', { name: '需求内容', exact: true }).waitFor()
  await page.screenshot({ path: '/tmp/flowlark-requirements-overview.png', fullPage: true })
  ...
  await page.getByRole('dialog').getByText('交付计划', { exact: true }).click()
  await page.getByLabel('目标交付', { exact: true }).fill('TBD')
  await page.getByRole('button', { name: /确\s*定/ }).click()
  await page.getByRole('tab', { name: '需求内容', exact: true }).click()
  await page.locator('.fl-requirement-content').getByText('修改后的背景', { exact: true }).waitFor()
  assert.equal(hub.getRequirement('REQ-FIELDS').rawDescription, '业务原文')
  assert.equal(hub.getRequirement('REQ-FIELDS').priority, 'P2')
  ...
  await page.getByRole('searchbox', { name: '搜索需求', exact: true }).click()
  ...
  await page.waitForLoadState('networkidle')
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth + 1), false)
  await page.getByRole('tab', { name: '来源与资料', exact: true }).click()
  await page.getByText('业务原文', { exact: true }).waitFor()
