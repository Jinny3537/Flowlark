...
当前版本：`v0.8.0`。除项目/版本外，已支持标注反馈、需求与迭代视图、交付快照、团队通知、只读镜像、跨仓库搜索、原型修改工作台、AI 草稿和需求平台接入。

需求与归档原型的角色协作、开发依据、反馈闭环及删除恢复，见 [需求协作指南](docs/REQUIREMENT-WORKFLOW.md)。原型在外部修改后导入归档，最新归档不会自动替代迭代采用版本。
...
```text
设为当前基线 → Git 提交/拉取/推送 → 发送企业微信发版邮件
```
...
wecom-cli auth init
wecom-cli auth show --status   # 应输出 authorized
```

随后在「项目 → 编辑项目 → 发版邮件」中配置默认收件人、抄送人、主题模板和 Markdown 正文模板。收件人填写企业微信成员姓名；正式发版预检会查询通讯录，同名时必须明确选择部门和职位对应的成员，不会猜测。

Flowlark 启动时会自动管理一个只监听 `127.0.0.1` 的企业微信 MCP Sidecar。Sidecar 通过官方 `wecom-cli` 查询联系人和发送邮件；Bot ID、Secret 与访问令牌仍由官方 CLI 加密管理，Flowlark 不读取也不写入仓库。临时邮件正文使用 `0600` 权限，发送完成后立即删除。

失败处理遵循“已经完成的步骤不倒退”：

- Git 同步失败：不发送邮件；修复 Git 后从同步步骤继续。
- 邮件失败：基线和 Git 不回滚，邮件进入本机待重试队列。
- 重复点击或浏览器重试：已确认发送成功的任务不会重复发送。
- 失败期间基线被切换到其他版本：停止自动重试，要求人工核对。
...
在「设置 → 团队协作」统一管理局域网访问地址和协作方式。「只读分享」允许浏览、下载；「角色协作」支持微信登录或游客访问，首次进入选择游客、研发或测试，再按角色提交评论、进度或测试结果。网络开关重启生效，协作方式立即生效；旧 `server.readonlyFromLan=false` 配置不能绕过角色权限。详见 [团队协作](docs/TEAM.md)。
...
设定基线仅变更版本状态。原型、变更日志、规格书、标签和附件均可继续编辑；废弃版本仍需先恢复。
...
基线不再锁定内容，也不要求变更日志或评审前置校验。设置中的发版规则仅影响正式发版流程。
...
抓取所有外链内联成一个自包含 HTML。**不修改原型文件本身**。离线版是派生产物，存在 `.flowlark/cache/` 下，随时可重新生成，也不进 Git。基线版本同样支持生成离线版。工作台里勾选「离线预览」即可切换。
...
| R4 | 基线与历史版本可继续编辑；已废弃版本需先恢复 |
| R5 | 版本号同项目内唯一，字符集受限（它同时是文件名） |
| R6 | 设定基线仅变更状态，不校验日志、评审结论或原型文件 |
| R7 | 删除是把文件移进 `.flowlark/trash/`，可恢复 |

已确认的当前基线也可废弃或删除，操作后清空当前基线，不自动切换到其他版本；恢复版本不会自动恢复基线身份，需重新设置。历史基线的废弃或删除不影响当前基线。
...
当前支持本机产品操作与远程游客、研发、测试角色。服务端同时检查协作方式、操作权限和仓库写权限；游客访问不提供账号身份认证，微信登录需配置网站应用凭据且不会提升角色权限，详见 [团队协作](docs/TEAM.md)。
...
截图 diff / 源码 diff · 自建账号体系 · 多人实时协同 · 云端托管 · 桌面客户端

需求支持本地维护和 MCP 导入同步；本地分析独立保留。源平台状态、原型进度、研发任务与验收记录分别展示。需求与迭代范围随 Git 同步。
