...
```bash
./start.sh ~/我的原型         # 指定数据仓库位置（默认 ~/protohub-repo）
...
```bash
cd protohub && npm link      # 之后 protohub 命令随处可用

cd ~/我的原型
protohub init
protohub new "订单中心重构" --code order-center
protohub add ./订单中心_v1.0.html -t "首版原型"
protohub baseline order-center v1.0
protohub open
```
...
**CLI 而不是上传按钮。** 原型就在你的文件系统里，`protohub add 文件.html -m "修改:筛选区:压缩为一行"` 一条命令归档完事，比打开浏览器拖拽快得多。CLI 也能进脚本、进 Makefile、进 CI。

**本地服务而不是内网服务器。** 没有部署、没有运维、没有账号系统、没有 CORS、没有防火墙工单。`protohub serve` 起在 localhost，关掉就没了，数据一直在你的目录里。
...
```bash
# 日常
protohub add <文件.html> [-n 版本号] -t "标题" [-m "类型:位置:说明[:需求号]"]... [--tag 已评审]
protohub ls [项目]                   # 版本时间线，带「新」标记与标签
protohub show <项目> <版本>           # 版本详情
protohub baseline <项目> <版本>       # 设为当前基线
protohub rollback <项目>             # 回滚到上一个基线
protohub spec <项目> <版本> --edit    # 用 $EDITOR 写规格书
protohub diff [项目]                 # 累计变更 + 反复修改热点

# 检索与组织
protohub search <关键词>             # 全库搜索
protohub read <项目> <版本>           # 标记「我看到这一版了」
protohub tag <项目> <版本> 已评审      # 版本标签
protohub attach <项目> <版本> <文件>  # 挂 PRD / 设计稿 / 评审纪要
protohub offline <项目> <版本>        # 生成自包含的离线版
protohub compare <项目> <v1> <v2>    # 浏览器里并排对比

# 系统
protohub config                     # 查看 / 修改全部配置
protohub lan on                     # 开放局域网，同事可访问
protohub remote <地址>               # 配置 Git 远端

# Git
protohub sync                       # 提交 + 拉取 + 推送
protohub history <项目> <版本>        # 这一版都谁改过
protohub blame <项目>                # 基线变迁史
protohub resolve                    # 冲突辅助解决

# 运行
protohub status / watch / serve / open
```
...
```
$ protohub diff order-center --from v1.0 --to v1.2
...
```bash
protohub read order-center v1.2   # 我看到这一版了
protohub ls order-center          # 之后的版本会被打上「新」
protohub diff order-center        # 只看这之后的变更
protohub status                   # 一屏看完哪些项目有你没看过的新版本
```

已读状态存在 `.protohub/cache/` 下，**不进 Git** —— 它是每个人自己的，提交上去会变成「张三把李四标成已读」这种荒唐冲突。
...
```bash
$ protohub blame order-center
基线    切换时间  操作人  提交     说明
● v1.2  2 小时前  张小雨  3d5aaed  B 切 v1.2
· v1.1  昨天      李哲    89169ff  A 切 v1.1
· v1.0  上周      张小雨  d395fe4  初始提交

$ protohub spec order-center v1.2 --history   # 规格书改过几次
$ protohub spec order-center v1.2 --at 0278f7d  # 回看当时的内容
```
...
▎基线冲突  projects/order-center/BASELINE
  两边各自把 order-center 的基线指向了不同版本：
    ...
    v1.1   （对方那边）
  选一个保留：
    protohub resolve order-center v1.2
    protohub resolve order-center v1.1
```
...
```bash
protohub lan on     # 开放给同网段
protohub serve      # 起服务，会打印同事该访问的地址
```
...
局域网用户看到的界面会显式标「只读」，写操作按钮直接隐藏——让人点了才收到 403 是很差的体验。真要多人可写就 `protohub config server.readonlyFromLan false`，设置页会用红字说明后果。

### 5. 附件 — PRD、设计稿、评审纪要

```bash
protohub attach order-center v1.2 ./需求文档.pdf ./评审纪要.md
```
...
```bash
protohub config                              # 分组列出全部
protohub config server.maxFileBytes 50MB     # 改
protohub config rules.requireChangelog       # 看单项详情与风险说明
```
...
```bash
protohub offline order-center v1.2
```

抓取所有外链内联成一个自包含 HTML。**不修改原型文件本身** —— 原型是需求追溯的证据，R4 说它确认后不可变。离线版是派生产物，存在 `.protohub/cache/` 下，随时可重新生成，也不进 Git。正因如此，基线版本也能生成离线版而不违反不可变性。工作台里勾选「离线预览」即可切换。
...
| R7 | 删除是把文件移进 `.protohub/trash/`，可恢复 |
...
浏览器工作台 `vite build` 通过（3198 模块）。CLI → 服务 → 浏览器 → 磁盘的完整链路做过端到端实跑，包括制造一次真实 Git 冲突并用 `protohub resolve` 解决。
...
8. **`sync` 会卷走用户放在旁边的文件。** 端到端跑完发现提交里混进了 `我正在改的原型.html`——`git add -A` 的必然结果。`protohub sync` 的契约应该是「同步 protohub 的数据」，不是「提交这个文件夹里的一切」。现在只暂存 `projects/`、`protohub.json` 等自有路径，跳过的文件会在输出里列出来。顺带修正了「工作区干净」的判断：旁边有草稿不该让同步按钮一直亮着。
...
- **无文件锁**。同一个仓库同时跑两个 `protohub serve` 并发写同一版本，后写的赢。本地单人场景不构成问题，多人靠 Git 而不是靠并发控制。
