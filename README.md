...
在「设置 → 团队协作」统一管理局域网访问地址和协作方式。「只读分享」允许浏览、下载；「角色协作」允许访客首次选择游客、研发或测试，再按角色提交评论、进度或测试结果。网络开关重启生效，协作方式立即生效；旧 `server.readonlyFromLan=false` 配置不能绕过角色权限。详见 [团队协作](docs/TEAM.md)。
...
当前支持本机产品操作与远程游客、研发、测试角色。服务端同时检查协作方式、操作权限和仓库写权限；浏览器记忆的角色不是账号身份认证，详见 [团队协作](docs/TEAM.md)。
