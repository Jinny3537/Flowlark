...
export const HELP = `${c.bold('protohub')} — 本地原型版本库

  ${c.dim('CLI 管数据，浏览器看原型。数据是纯文本文件，直接进 Git。')}

${c.bold('起步')}
  protohub init                       在当前目录创建仓库
  protohub new "订单中心重构"          新建项目
  protohub add 原型.html -n v1.0 -t "首版"
  protohub open                       起服务并打开浏览器工作台
...
${c.dim('查看单条命令的详细用法：protohub help <命令>')}
${c.dim('全局：-h 帮助  -V 版本  --json 机器可读输出  NO_COLOR=1 关闭颜色')}
`

export const COMMAND_HELP = {
  init: `${c.bold('protohub init')} [目录]
...
`,
  ...
  new: `${c.bold('protohub new')} <项目名> [选项]

  --code <标识>    项目目录名，小写字母/数字/连字符。默认从项目名生成
  --desc <描述>

示例
  protohub new "订单中心重构" --code order-center --desc "列表与详情改版"
`,

  add: `${c.bold('protohub add')} <HTML文件> [选项]
...
示例
  protohub add ./订单中心_v1.1.html -n v1.1 -t "批量操作首版" \\
...
`,
  ...
  baseline: `${c.bold('protohub baseline')} <项目> <版本号>
...
`,
  ...
  rollback: `${c.bold('protohub rollback')} <项目>

回滚到上一个当过基线的版本。用于新版评审没过、需要让研发退回上一版的场景。
不会要求补写变更日志 —— 回滚是止血动作，不是向前推进。
`,

  change: `${c.bold('protohub change')} <项目> <版本号> -m "类型:位置:说明[:需求号]"

给版本追加一条变更日志。只有「编辑中」的版本能改。

示例
  protohub change order-center v1.2 -m "修改:订单详情-头部:金额字号放大到 24px"
`,

  spec: `${c.bold('protohub spec')} <项目> <版本号> [选项]
...
示例
  protohub spec order-center v1.2 --history
  protohub spec order-center v1.2 --at a1b2c3d
`,

  diff: `${c.bold('protohub diff')} [项目] [选项]

  --from <版本号>   起点
  --to <版本号>     终点，默认当前基线

不带 --from/--to 时，起点取${c.bold('你上次标记已读的那一版')}（见 protohub read），
没有已读记录则退化为「比上一版改了什么」。
...
`,
  ...
  search: `${c.bold('protohub search')} <关键词> [选项]
...
示例
  ...
  protohub search 幂等 --field spec
`,

  read: `${c.bold('protohub read')} <项目> [版本号]

  不给版本号   查看当前已读状态，以及之后新增了几版
  <版本号>     标记为已读；用 . 表示当前基线
  --clear      清除标记

已读状态存在 .protohub/cache/ 下，${c.bold('不进 Git')} —— 它是每个人自己的，
提交上去会变成「张三把李四标成已读」这种荒唐冲突。

标记之后 ${c.cyan('protohub ls')} 会给新版本打「新」，${c.cyan('protohub diff')} 会从这一版算起。
`,

  tag: `${c.bold('protohub tag')} [项目] [版本号] [标签…]
...
`,
  ...
  offline: `${c.bold('protohub offline')} <项目> <版本号> [--clear]

抓取原型引用的 CDN 资源并内联，生成一个自包含的 HTML。
断网、上高铁、代理拦截时用它预览，样式不会掉。

${c.bold('不会修改原型文件本身')} —— 原型是需求追溯的证据，确认后不可变。
离线版是派生产物，存在 .protohub/cache/offline/ 下，不进 Git，随时可重新生成。
正因为如此，基线版本也能生成离线版而不违反不可变性。
`,

  compare: `${c.bold('protohub compare')} <项目> [版本A] [版本B]
...
`,
  ...
  sync: `${c.bold('protohub sync')} [选项]

  -m, --message <说明>   提交信息。不给会按改动内容自动生成
      --no-push          只提交和拉取，不推送

${c.bold('只提交 protohub 自己的路径')}（projects/、protohub.json 等），
...
产生冲突时会提示用 ${c.cyan('protohub resolve')} 处理。
`,

  history: `${c.bold('protohub history')} <项目> [版本号]

这一版都被谁改过、每次改的是元数据、规格书还是原型文件本身。
底下是 git log，但按版本聚合过：同一次提交同时改了元数据和规格书时会合成一条。

配合 ${c.cyan('protohub spec <项目> <版本> --at <提交号>')} 可以回看当时的规格书。
`,

  blame: `${c.bold('protohub blame')} <项目>
...
`,
  ...
  resolve: `${c.bold('protohub resolve')} [项目] [版本号]
...
`,
  ...
  watch: `${c.bold('protohub watch')} <项目> -d <目录>
...
`,
  ...
  attach: `${c.bold('protohub attach')} <项目> <版本号> [文件…]
...
`,
  ...
  config: `${c.bold('protohub config')} [配置项] [值]

  不带参数           分组列出全部配置
  <配置项>           查看单项详情、默认值与风险说明
  <配置项> <值>       修改
  <配置项> --clear   恢复默认
  --edit             用 $EDITOR 直接改 protohub.json

配置存在仓库根目录的 protohub.json 里，${c.bold('随 Git 提交，团队共用同一份')}。

标 ${c.red('!')} 的是高风险开关 —— 关掉会削弱产品的核心保证（如 R4 基线锁定、R6 变更日志必填）。

示例
  protohub config server.maxFileBytes 50MB
  protohub config ui.requirementUrlTemplate "https://jira.internal/browse/{code}"
`,

  lan: `${c.bold('protohub lan')} [on|off]
...
真要多人可写：protohub config server.readonlyFromLan false
`,

  remote: `${c.bold('protohub remote')} [地址]

  不带参数    查看当前远端与同步状态
  <地址>      设置远端（已存在则改地址）
  --clear     移除远端

配置后 ${c.cyan('protohub sync')} 会自动推送，首次推送自动建立上游分支。

顺带会设 core.quotepath=false，让中文文件名在 git 输出里正常显示 ——
Git 默认把非 ASCII 文件名转义成八进制，中文项目里 git status 全是乱码。
`,

  serve: `${c.bold('protohub serve')} [选项]
...
`,
  ...
  open: `${c.bold('protohub open')} [项目] [版本号]
...
`,
  ...
  status: `${c.bold('protohub status')}

一屏看完：项目数、版本数、Git 分支与同步状态、冲突、缺基线的项目、
以及${c.bold('哪些项目有你没看过的新版本')}。

最后一项配合 ${c.cyan('protohub read')} 使用，是研发每天上班第一条命令。
`
}
