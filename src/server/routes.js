...
export function buildApi(hub, { previewPort, runtime = {} }) {
  ...
  r.post('/api/milestones/:name/sync', async (req, res, p) => {
    await readJson(req, maxBody)
    sendJson(res, 200, await hub.planMilestoneSync(p.name, {}))
  })
  ...
  r.post('/api/mcp/requirement-pool/status', async (req, res) => {
    const body = await readJson(req, maxBody)
    sendJson(res, 200, await hub.requirementPoolStatus({ probe: body.probe === true }))
  })
