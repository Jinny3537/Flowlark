...
- Real-platform smoke results now carry auditable release evidence: `generatedBy`, `mode: live`, `evidenceVersion: v075-requirement-pool-smoke/v1`, manifest summary, connection proof and required path checks. The readiness gate rejects legacy minimal smoke JSON that only contains `passed`, `requirement`, `snapshot` and source metadata. Focused verification after adding the evidence contract: `node --check scripts/smoke-v075-requirement-pool.mjs && node --check scripts/check-v075-readiness.mjs` passed and `node --test test/v07-upgrade.test.js` passed 11/11.
