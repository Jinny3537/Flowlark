...
- `upgrade:v075` now provides the one-command upgrade orchestration. With a real manifest, credentials and Playwright it runs Web build, browser smoke, real-platform smoke, pre-bump readiness, guarded finalizer and final readiness; it also supports explicit reuse of already saved smoke evidence. Focused verification after adding the orchestrator: `node --check scripts/upgrade-v075.mjs && node --check scripts/check-v075-readiness.mjs && node --check scripts/finalize-v075-release.mjs` passed and `node --test test/v07-upgrade.test.js` passed 12/12 using a temporary package tree and reused accepted smoke evidence.
