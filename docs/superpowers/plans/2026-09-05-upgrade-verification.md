...
- MCP Center now has a minimal file-or-paste preview UI for requirement-pool configuration JSON. It can load the canonical starter template, validates uploaded JSON file type and a 256KB size limit, shows platform/server/tool/secrets summary, blockers and warnings, imports only when the preview is not blocked, and provides a separate integration status/connection-test check.
...
- Focused verification for requirement-pool template generation, HTTP template routing and MCP settings model parsing: 57/57 passed.
- Full suite after the requirement-pool configuration foundation, MCP Center import UI, delivery source freezing, integration diagnostics, single-requirement refresh, requirement-pool list refresh, remote-missing marking, manifest file selection and canonical template loading: 706/706 passed, zero failures.
- `npm run build:web` passed. Existing dependency audit notices (one moderate, one high) and Vite bundle-size warning remain.
