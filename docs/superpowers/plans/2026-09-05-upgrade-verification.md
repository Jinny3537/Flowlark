...
- `scripts/smoke-v075-mcp-ui.mjs` now defines a browser smoke for the default Requirements-page requirement-pool path and advanced MCP Settings path: template load, manifest preview/import, missing secret UI, environment-secret connection probe, requirement search/import, page errors and desktop/mobile overflow. It intentionally does not click the Keychain save action, to avoid writing secrets to the operator machine during smoke verification.
...
- The Requirements import dialog now provides the default v0.7.5 configuration path itself: load a template, paste or choose a requirement-pool manifest JSON, preview blockers/warnings, import the MCP configuration, fill missing local secrets and run a read-only connection test before searching external requirements. The MCP Center remains the advanced settings and diagnostics path.
...
- Focused verification after adding the direct requirement-page configuration import and local credential/probe path: `npm run build:web` passed, `node --test test/v07-upgrade.test.js test/mcp-config.test.js web/src/pages/requirementsModel.test.js` passed 32/32, and `PLAYWRIGHT_MODULE=/tmp/flowlark-playwright-s3dUGW/node_modules/playwright/index.mjs PLAYWRIGHT_BROWSERS_PATH=/tmp/flowlark-playwright-s3dUGW/browsers npm run smoke:v075:mcp-ui` passed with `requirements-direct-config-import`, `requirements-secret-ui`, `requirements-env-secret-probe`, `settings-advanced-entrypoint`, manifest preview/import, missing-secret UI, env-secret probe, desktop/mobile layout and zero page errors.
- Full suite after adding the direct requirement-page configuration import and local credential/probe path: `node --test` passed 714/714, zero failures.
- Browser smoke after adding requirement search/import to the default Requirements-page path: `node --check scripts/smoke-v075-mcp-ui.mjs` passed and `PLAYWRIGHT_MODULE=/tmp/flowlark-playwright-s3dUGW/node_modules/playwright/index.mjs PLAYWRIGHT_BROWSERS_PATH=/tmp/flowlark-playwright-s3dUGW/browsers npm run smoke:v075:mcp-ui` passed with `requirements-search-import`, verified authenticated `requirements.search`/`requirements.get` calls and opened the imported requirement detail.
- Real-platform smoke harness now verifies that the formal delivery snapshot freezes a complete requirement-pool source summary (`code`, title, source, provider, external key, status, sync time and source URL when returned by the platform) instead of only checking that a source key exists. Focused verification after strengthening the snapshot-source assertions: `node --check scripts/smoke-v075-requirement-pool.mjs` passed and `node --test test/v07-upgrade.test.js` passed 9/9.
- Real-platform smoke harness now reads back the imported requirement detail and created milestone after linking the test requirement to the prototype version and adding it to the iteration scope. This proves the association is visible through product APIs before formal delivery. Focused verification after adding the read-back assertions: `node --check scripts/smoke-v075-requirement-pool.mjs` passed and `node --test test/v07-upgrade.test.js` passed 9/9.
- Real-platform smoke results now carry auditable release evidence: `generatedBy`, `mode: live`, `evidenceVersion: v075-requirement-pool-smoke/v1`, manifest summary, manifest fingerprint, connection proof and required path checks. The readiness gate rejects legacy minimal smoke JSON that only contains `passed`, `requirement`, `snapshot` and source metadata, and rejects smoke evidence whose manifest fingerprint does not match the current manifest. Focused verification after adding the evidence contract: `node --check scripts/smoke-v075-requirement-pool.mjs && node --check scripts/check-v075-readiness.mjs` passed and `node --test test/v07-upgrade.test.js` passed 13/13.
- `check:v075:readiness` now provides a read-only release gate for the final v0.7.5 handoff, with `pre-bump` and `final` phases. It checks package and web package versions in the final phase, npm smoke entrypoints, manifest availability and blockers, derived credential environment variables, Playwright availability, saved real-platform smoke evidence and saved browser smoke evidence. The real-platform and browser smoke scripts accept `--output` to persist that evidence. Focused verification after adding the gate: `node --check scripts/check-v075-readiness.mjs && node --check scripts/smoke-v075-requirement-pool.mjs` passed, `node --test test/v07-upgrade.test.js` passed 10/10, and `npm run check:v075:readiness` currently fails as expected because final version bump, real manifest, Playwright module and saved smoke evidence are still missing.
- `release:v075:finalize` now automates the final version bump after pre-bump readiness passes. It updates `package.json`, `package-lock.json`, `web/package.json` and `web/package-lock.json` to `0.7.5`, then reruns final readiness. The readiness gate also checks lockfile root versions. Focused verification after adding the finalizer: `node --check scripts/finalize-v075-release.mjs && node --check scripts/check-v075-readiness.mjs` passed and `node --test test/v07-upgrade.test.js` passed 11/11 against a disposable temporary package tree.
- Current local UI smoke evidence was regenerated with `PLAYWRIGHT_MODULE=/tmp/flowlark-playwright-s3dUGW/node_modules/playwright/index.mjs PLAYWRIGHT_BROWSERS_PATH=/tmp/flowlark-playwright-s3dUGW/browsers npm run smoke:v075:mcp-ui -- --output .flowlark/cache/v075-mcp-ui-smoke.json` and passed. With the example manifest, dummy local secret and saved UI evidence, `npm run check:v075:readiness -- --phase pre-bump --manifest docs/examples/requirement-pool-manifest.example.json --ui-smoke-result .flowlark/cache/v075-mcp-ui-smoke.json` now fails only on `real-smoke-result`, and its `next` list only asks for the real-platform smoke.
- `upgrade:v075` now provides the one-command upgrade orchestration. With a real manifest, credentials and Playwright it first requires a clean Git worktree, then runs Web build, browser smoke, real-platform smoke, pre-bump readiness, guarded finalizer and final readiness; it also supports explicit reuse of already saved smoke evidence. The orchestrator reports argument and preflight failures as JSON, and preflight checks that reusable real/UI smoke result files exist before continuing. `release:v075:finalize` also refuses to write version files in a dirty Git worktree. Focused verification after hardening the orchestrator: `node --check scripts/upgrade-v075.mjs && node --check scripts/check-v075-readiness.mjs && node --check scripts/finalize-v075-release.mjs` passed and `node --test test/v07-upgrade.test.js` passed 14/14 using a temporary package tree and reused accepted smoke evidence.
- `upgrade:v075`, `release:v075:finalize` and `check:v075:readiness` now accept `--playwright-module` and forward it through the readiness/finalization chain, so operators do not have to export `PLAYWRIGHT_MODULE` globally. Focused verification after adding the argument path: `node --check scripts/upgrade-v075.mjs && node --check scripts/check-v075-readiness.mjs && node --check scripts/finalize-v075-release.mjs` passed and `node --test test/v07-upgrade.test.js` passed 14/14 with `PLAYWRIGHT_MODULE` intentionally blank in the relevant child process environments.
- `upgrade:v075` and `check:v075:readiness` now auto-detect Playwright from root `node_modules`, `web/node_modules`, and local `/tmp/flowlark-playwright-*` installs before requiring `--playwright-module` or `PLAYWRIGHT_MODULE`. Focused verification after adding auto-detection: `node --check scripts/upgrade-v075.mjs && node --check scripts/check-v075-readiness.mjs` passed and `node --test test/v07-upgrade.test.js` passed 15/15 with a fake local `node_modules/playwright/index.mjs` and blank `PLAYWRIGHT_MODULE`.
- `upgrade:v075` and `release:v075:finalize` now include a parsed `childResult` when readiness child steps fail, so release operators can see the exact failed readiness checks from the top-level JSON output. Focused verification after adding child-result passthrough: `node --check scripts/upgrade-v075.mjs && node --check scripts/finalize-v075-release.mjs` passed and `node --test test/v07-upgrade.test.js` passed 15/15.
...
- `npm run build:web` passed. Existing dependency audit notices (one moderate, one high) and Vite bundle-size warning remain.
- HubPooL compatibility was added for v0.7.5 configuration JSON import: explicit `transport.headers: {}` now means no-auth local/internal MCP instead of silently injecting an Authorization placeholder; requirement MCP calls now send both generic and HubPooL-style aliases (`keyword`, `projectId`, `requirementId`) and unwrap detail payloads from `requirement`/`item`/`data`/`result`.
- Real HubPooL smoke passed against the local platform at `http://127.0.0.1:8080/mcp` using project `proj_1784809663328` and requirement `REQ-HFHT-202607-003`. Evidence was saved to `.flowlark/cache/v075-requirement-pool-smoke.json` with `generatedBy: smoke:v075:requirement-pool`, `mode: live`, `evidenceVersion: v075-requirement-pool-smoke/v1`, manifest fingerprint `dbfa98d87a3f13847230a31d5e20e3a16847202477483faa00f36d46bde3e803`, connected platform `HubPooL`, version `v0.7.5-live`, milestone `V075-LIVE` and delivery snapshot `delivery-24c53545ef53d1633e5bd15e047cdec54f1ad4a35da348bb`.
- Current final verification pass for the v0.7.5 upgrade: `node --test test/mcp-config.test.js test/v07-upgrade.test.js` passed 34/34, `PLAYWRIGHT_MODULE=/tmp/flowlark-playwright-2xGrMK/node_modules/playwright/index.mjs PLAYWRIGHT_BROWSERS_PATH=/tmp/flowlark-playwright-2xGrMK/browsers npm run smoke:v075:mcp-ui -- --output .flowlark/cache/v075-mcp-ui-smoke.json` passed, `npm run smoke:v075:requirement-pool -- --manifest .flowlark/cache/v075-hubpool-manifest.json --query '' --requirement REQ-HFHT-202607-003 --output .flowlark/cache/v075-requirement-pool-smoke.json` passed, `npm run check:v075:readiness -- --manifest .flowlark/cache/v075-hubpool-manifest.json --smoke-result .flowlark/cache/v075-requirement-pool-smoke.json --ui-smoke-result .flowlark/cache/v075-mcp-ui-smoke.json --playwright-module /tmp/flowlark-playwright-2xGrMK/node_modules/playwright/index.mjs` passed in final phase, `npm run build:web` passed, `git diff --check` passed and the full `node --test` suite passed 722/722 with zero failures.
- Package versions were raised to `0.7.5` in `package.json`, `package-lock.json`, `web/package.json` and `web/package-lock.json` after final readiness evidence was available.

v0.7.5 exit status: complete for the agreed first priority, namely configuration JSON based integration with the existing requirement-pool platform. Service-package/stdio upload remains deliberately out of v0.7.5 scope and is planned for the follow-up v0.7.8 track.
