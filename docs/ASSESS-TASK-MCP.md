...
同步中心只暴露服务端固定的“执行”、“重试”和“取消”动作。浏览器可提交计划哈希、原因和未完成任务确认，但不能指定 MCP 服务、工具名或任意远端操作。执行时，服务端从记录取出 `entityKey` 和 `planHash`，用 `entityKey` 重新读取当前本地数据与远端对象并重建计划；只有客户端提交哈希、记录哈希和重建后的计划哈希一致时才会执行，不会直接信任存储的计划内容。
...
审计记录会递归脱敏密码、授权信息、Token、Secret 和环境变量值，然后以 append-only 文本进入 Git。
