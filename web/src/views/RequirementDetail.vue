<template>
  <div class="page-pad detail-page" v-if="item">
    ...
    <h3>跨项目版本演进</h3><a-table :data="item.versions" row-key="versionNo" size="middle" :pagination="false" :scroll="{ x: 760 }">
      <template #columns>
        <a-table-column title="项目" data-index="project" :width="160" />
        <a-table-column title="版本">
          <template #cell="{ record }"><a @click="$router.push(`/projects/${record.project}/versions/${record.versionNo}`)">{{ record.versionNo }} · {{ record.title }}</a></template>
        </a-table-column>
        <a-table-column title="基线" :width="120">
          <template #cell="{ record }"><a-tag :color="record.isBaseline?'green':'default'">{{ record.isBaseline?'当前基线':'非基线' }}</a-tag></template>
        </a-table-column>
