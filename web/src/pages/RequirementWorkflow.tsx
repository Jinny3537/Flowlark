...
export default function RequirementWorkflow({ item, writable, onChanged }: { item: any; writable: boolean; onChanged: () => void }) {
  ...
  return <div className="fl-detail-stack">
    ...
    <section id="requirement-iterations" className="fl-detail-section"><h2>本次开发依据</h2>
      ...
      <List dataSource={item.milestones || []} locale={{ emptyText: '尚未安排迭代，开发依据未确定。请在迭代中加入该需求及归档版本。' }} renderItem={(m: any) => <List.Item>
        <Space direction="vertical"><Space><Link to={contextualRoute(`/milestones/${encodeURIComponent(m.name)}`, location.pathname + location.search, `需求 ${item.code}`)}>{m.title}</Link><Tag>{phase[m.status] || m.status}</Tag></Space>
      ...
      <Space wrap>{!item.versions?.length && <Button disabled={!editable} onClick={() => void openLink()}>先关联归档原型</Button>}<Button disabled={!editable || !item.versions?.length} loading={busy} onClick={() => setIterationOpen(true)}>安排到迭代</Button><Link to="/milestones">管理迭代与采用范围</Link></Space>
    </section>
    <section id="requirement-archives" className="fl-detail-section"><Space wrap><h2>归档原型与变更</h2><Button disabled={!editable} loading={busy} onClick={() => void openLink()}>关联已有归档</Button></Space>
    ...
    <AssignIterationDialog open={iterationOpen} onClose={() => setIterationOpen(false)} source={{ requirement: item.code }} writable={editable} onChanged={onChanged} />
    <Modal title="关联归档原型" open={linkOpen} onCancel={() => setLinkOpen(false)} onOk={() => void link()} confirmLoading={busy}>
      <p>基线仅标识当前版本，仍可编辑；需要保留独立变更记录时，可归档新版本并关联本次变更。</p>
