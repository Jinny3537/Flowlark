...
# flowlark 一键启动
#
#   ./start.sh              数据仓库默认放在 ~/flowlark-repo
...
REPO="${REPO:-$HOME/flowlark-repo}"

echo
echo "${B}flowlark${N} ${DIM}· 本地原型版本库${N}"
...
if command -v git >/dev/null 2>&1; then
  ...
  warn "没有 git。flowlark 本身能用，但团队协作依赖 Git 共享数据目录"
fi
...
if [ -f "$REPO/flowlark.json" ]; then
  ok "使用已有仓库 ${C}$REPO${N}"
else
  mkdir -p "$REPO"
  node bin/flowlark.js init "$REPO" >/dev/null
  ok "已创建仓库 ${C}$REPO${N}"
...
PROTO
  ...
  export FLOWLARK_REPO="$REPO"
  node bin/flowlark.js new "订单中心重构" --code order-center --desc "示例项目，可随时删除" >/dev/null
  node bin/flowlark.js add "$SAMPLE/订单中心_v1.0.html" -p order-center -n v1.0 -t "首版原型" >/dev/null
  node bin/flowlark.js baseline order-center v1.0 >/dev/null
  node bin/flowlark.js add "$SAMPLE/订单中心_v1.1.html" -p order-center -n v1.1 -t "批量操作首版" \
  ...
  node bin/flowlark.js spec order-center v1.1 -f /dev/stdin >/dev/null <<'SPEC'
# 批量操作首版 · 规格说明
...
echo "  ${DIM}  node ${HERE}/bin/flowlark.js ls${N}"
echo "  ${DIM}  node ${HERE}/bin/flowlark.js add <文件.html> -t \"标题\"${N}"
echo
echo "  ${DIM}想让 flowlark 命令全局可用：cd ${HERE} && npm link${N}"
echo
...
exec node "$HERE/bin/flowlark.js" open $PORT_ARG $OPEN_ARG $LAN_ARG
